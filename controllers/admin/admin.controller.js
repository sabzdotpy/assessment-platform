...
export const addBulkUserFromCSV = async (req, res) => {
  try {
    if (!req.file) {
      ...
      return Response.error(res, 400, "CSV file is required.");
    }
    ...
    if (!Array.isArray(users) || users.length === 0) {
      ...
      return Response.error(res, 400, "No valid users found in CSV.");
    }
    await bulkCreateCandidates(users);
    // res.status(201).json({
    //   message: "Candidates created and credentials sent successfully.",
    // });
    return Response.success(res, 200, "Bulk users added from CSV.", users);
  } catch (error) {
    // console.error(error);
    // res.status(500).json({ message: "Error processing CSV", error });
    return Response.error(res, 500, "Error processing CSV", error);
  }
};
